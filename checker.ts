...
function problem14() {
  return prisma.$queryRaw`SELECT CAST(SUM(salary) as CHAR(7)) as "sum of employees salaries" from
  Employee e
      join Branch b on e.branchNumber = b.branchNumber
...
function problem15() {
  return prisma.$queryRaw`SELECT customerID, firstName, lastName from
  Customer c
  where (SELECT count(distinct b.branchName) from
          Owns o
              join Account a on o.accNumber = a.accNumber
              join Branch b on a.branchNumber = b.branchNumber
          where c.customerID=o.customerID)=4
...
function problem18() {
  return prisma.$queryRaw`SELECT a.accNumber, a.balance, SUM(t.amount) as "sum of transaction amounts" from
  Account a
